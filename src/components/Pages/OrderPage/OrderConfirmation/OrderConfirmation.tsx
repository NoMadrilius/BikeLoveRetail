...
const OrderConfirmation = (p: { order: OrderFullData }) => {
  return (
    <section className="bg-white py-6 px-5 rounded-lg flex flex-col gap-5 mob:mt-6 mob:px-5 mob:pb-8">
      ...
      <AutorizationDataInfo
        phone={p.order.client.phoneNumber}
        name={
          p.order.client.lastName +
          " " +
          p.order.client.firstName +
          " " +
          p.order.client.patronymic
        }
        className="w-full"
      />
      <div className="flex flex-col gap-4">
        ...
        <DeliveryInfo
          ...
          delInfo={
            p.order.order.deliveryType === "DeliveryNP"
              ? "Доставка новою поштою"
              : "Самовивіз з магазину Щербаківського 59 10:00-18:00"
          }
        />
      </div>
