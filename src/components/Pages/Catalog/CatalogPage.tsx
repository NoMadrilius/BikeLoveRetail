...
const CatalogPage = () => {
    ...
    let page = cs.catalogState!.page != 1 ?` - Сторінка ${cs.catalogState!.page.toString()}`:''
    ...
    return (
        <div className="max-w-[1324px] w-full m-auto items-start my-10 flex flex-col gap-10">
            ...
            <CategoryName title={cs.catalogState?.category?.name ?? "no data"} quantity={cs.catalogState?.totalProducts ?? 666} />
            <SubCategories />
